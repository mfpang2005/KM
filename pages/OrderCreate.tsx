            ...
            <main className="flex-1 max-w-[1600px] mx-auto w-full px-4 lg:px-8 py-6 lg:py-10">
                <div className="flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-12">
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h2 className="text-sm font-black uppercase tracking-widest text-slate-800">选择菜色</h2>
                                </div>
                            </div>
                            <div className="flex flex-col gap-6">
                                <div className="flex flex-col md:flex-row gap-4">
                                    <div className="flex-1 relative group">
                                        ...
                                        <input
                                            type="text"
                                            className="w-full pl-12 pr-10 py-4 bg-white border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.02)] rounded-2xl text-sm font-bold outline-none focus:ring-2 focus:ring-primary/5 transition-all"
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            className="w-full px-6 py-4 bg-white border border-slate-100 rounded-2xl text-sm font-bold outline-none shadow-[0_4px_20px_rgb(0,0,0,0.02)] md:w-48"
                                            placeholder="输入编号"
                                            value={manualCode}
                                            onChange={e => setManualCode(e.target.value)}
                                            onKeyDown={e => e.key === 'Enter' && handleManualInput()}
                                        />
                                        <button onClick={handleManualInput} className="px-8 bg-slate-900 text-white rounded-2xl font-bold text-sm active:scale-95 transition-all">添加</button>
                                    </div>
                                </div>
                                <div className="flex gap-3 overflow-x-auto no-scrollbar py-2">
                                    {CATEGORIES.map(cat => (
                                        <button
                                            ...
                                            className={`px-6 py-2.5 rounded-full text-xs font-black transition-all whitespace-nowrap border-2 ${activeCategory === cat ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white text-slate-400 border-slate-50 hover:border-slate-200'}`}
                                        >
                                            {cat}
                                        </button>
                                    ))}
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                                    {productsLoading ? (
                                        <div className="col-span-full py-20 flex flex-col items-center justify-center text-slate-300 gap-4">
                                            ...
                                            <span className="text-sm font-black uppercase tracking-widest">初始化菜单...</span>
                                        </div>
                                    ) : filteredMenu.length === 0 ? (
                                        <div className="col-span-full py-20 bg-white rounded-[40px] border border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-300">
                                            <span className="material-icons-round text-6xl mb-4">no_food</span>
                                            <p className="font-bold text-sm">未能找到相关菜品</p>
                                        </div>
                                    ) : filteredMenu.map(p => (
                                        <div key={p.id} className="group bg-white rounded-[32px] overflow-hidden border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.06)] transition-all duration-500 hover:-translate-y-1 relative">
                                            <div className="aspect-[4/3] overflow-hidden relative">
                                                <div className="absolute top-4 left-4 z-10"><span className="px-2 py-1 bg-black/60 backdrop-blur-md text-[9px] text-white rounded font-black uppercase tracking-widest">{p.code}</span></div>
                                                <img src={p.img || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&h=300&fit=crop'} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt={p.name} />
                                            </div>
                                            <div className="p-5 space-y-4">
                                                ...
                                                <div className="flex justify-between items-center pt-2">
                                                    ...
                                                    <button onClick={() => addToCart(p)} className="w-10 h-10 rounded-full bg-primary text-white shadow-lg shadow-primary/20 active:scale-90 transition-all flex items-center justify-center"><span className="material-icons-round">add</span></button>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                    ...
                    <div className="lg:col-span-4 lg:sticky lg:top-32 lg:h-fit">
                        <section className="bg-white rounded-[40px] shadow-[0_20px_50px_rgb(0,0,0,0.06)] border border-slate-100 flex flex-col max-h-[calc(100vh-160px)]">
                            <div className="p-8 border-b border-slate-50 flex items-center justify-between">
                                <h3 className="text-base font-black text-slate-900 border-l-4 border-primary pl-4">订单明细</h3>
                                <span className="bg-slate-100 text-slate-500 px-3 py-1 rounded-full text-[10px] font-black uppercase">{cart.length} 项</span>
                            </div>
                            ...
                            <div className="p-8 bg-slate-50/50 border-t border-slate-100 rounded-b-[40px] space-y-6">
                                <div className="flex justify-between items-end">
                                    <div className="flex flex-col"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-2">应付合计</span><div className="flex items-baseline gap-1"><span className="text-sm font-black text-primary">RM</span><span className="text-4xl font-black text-primary tracking-tighter leading-none">{totalPrice.toFixed(2)}</span></div></div>
                                </div>
                                <button
                                    onClick={handleFinalConfirm}
                                    disabled={cart.length === 0 || isSubmitting}
                                    className={`w-full py-5 rounded-3xl font-black text-sm uppercase tracking-widest shadow-2xl transition-all active:scale-[0.98] flex items-center justify-center gap-3 ${cart.length > 0 && !isSubmitting ? 'bg-primary text-white shadow-primary/30' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
                                >
                                    {isSubmitting ? '正在处理...' : '发送并确认订单'}
                                    {!isSubmitting && <span className="material-icons-round text-base">arrow_forward</span>}
                                </button>
                            </div>
                        </section>
                    </div>
