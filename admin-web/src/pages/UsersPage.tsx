    ...
    return (
        <div className="mt-10 mx-auto max-w-[1600px] px-4 min-h-screen bg-[#FDFDFF] pb-20">
            ...
            <div className={`sticky top-0 z-[40] transition-all duration-500 ease-in-out ${
                isScrolled 
                ? 'bg-white/80 backdrop-blur-2xl border-b border-slate-100 py-2 shadow-sm' 
                : 'bg-transparent py-5'
            }`}>
            ...
            <div className="px-6">
                {/* Optimized Search Bar - Controlled Position */}
                <div className={`sticky z-30 transition-all duration-500 mb-8 mx-auto max-w-7xl ${
                    isScrolled ? 'top-[78px]' : 'top-6'
                }`}>
                    <div className="bg-white/60 backdrop-blur-3xl p-1.5 rounded-[22px] border border-white/80 shadow-2xl shadow-slate-200/40 flex flex-col md:flex-row gap-1.5 items-center">
                        <div className="relative flex-1 w-full group">
                            <span className="absolute left-5 top-1/2 -translate-y-1/2 material-icons-round text-slate-300 text-[18px] group-focus-within:text-blue-500 transition-colors">search</span>
                            <input
                                type="text"
                                placeholder="搜索姓名、邮箱或工号..."
                                value={searchQuery}
                                onChange={e => setSearchQuery(e.target.value)}
                                className="w-full pl-12 pr-6 py-3 bg-white/40 border-2 border-transparent rounded-[18px] text-xs focus:bg-white focus:border-blue-500/10 transition-all font-bold placeholder:text-slate-200"
                            />
                        </div>
                        <button
                            onClick={() => { setSearchQuery(''); window.history.replaceState({}, '', window.location.pathname); }}
                            className="h-[44px] px-5 bg-white border border-slate-50 flex items-center justify-center rounded-[18px] text-slate-400 font-black text-[9px] uppercase tracking-widest hover:text-red-500 hover:border-red-100 transition-all shadow-sm active:scale-95"
                        >
                            Reset
                        </button>
                    </div>
