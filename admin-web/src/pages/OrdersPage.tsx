...
export const OrdersPage: React.FC = () => {
    ...
    const handleAssignDriver = async (orderId: string, driverId: string) => {
        try {
            setIsSubmitting(true);
            await SuperAdminService.assignDriver(orderId, driverId);
            await loadOrders();
                                        ...
                                        <tr className="hover:bg-slate-50 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 pr-8 text-right space-x-1.5 whitespace-nowrap">
                                                <button
                                                    ...
                                                    title={order.driverId ? `Driver: ${fleetStatus.find(d => d.id === order.driverId)?.name || 'Assigned'}` : "Assign Driver"}
                                                    className={`w-8 h-8 inline-flex items-center justify-center rounded-lg hover:scale-110 transition-all shadow-sm group ${
                                                        order.driverId
                                                ...
                                                {order.driverId && (
                                                    <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-1 rounded-md border border-blue-100 hidden lg:inline-block align-middle mr-1">
                                                        {fleetStatus.find(d => d.id === order.driverId)?.name?.split(' ')[0] || 'Driver'}
                                                    </span>
                                                )}
